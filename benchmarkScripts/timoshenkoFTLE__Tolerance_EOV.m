...
for i=1:nTolerances
    ...
    for j = 1:nPoints
        ...
        tic;
        [eigmax, ~] = computeCGInvariants(dy, dygrad, maxPlace.', [0, 1.5], 'finiteDifference', false, tol);
        Times(i,j) = toc;
        FTLES(i,j) = log(eigmax)/(3);
    end
end

save('ftle_timoshenko_sensitivity_to_tolerance_withJacobian.mat', 'Times', 'FTLES');
