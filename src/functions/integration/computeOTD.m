function cgEigenvalueMax = computeOTD(derivative, initialPosition, timeSpan,r, dT)
...
Tp = OTDEquation(derivative, initialPosition, timeSpan, r, dT);
...
function Tp = OTDEquation(derivative, initialPosition, timeSpan, r, dT)
    ...
    derivNonEov = @(t,x) derivative(t,x,0,false);
    ...
    L = gradientL(derivEov, t0, initialPosition, nSystem, nRows); %% derivative at the initial time instant
    ...
    for k=1:kmax %Runge-Kutta steps
        ...
        K1 = Velocity_Field(derivNonEov,t(k), Xp, nSystem, nRows);
        K2 = Velocity_Field(derivNonEov, t(k)+dT/2, Xp+dT*K1/2,  nSystem, nRows);
        K3 = Velocity_Field(derivNonEov, t(k)+dT/2, Xp+dT*K2/2,  nSystem, nRows);
        K4 = Velocity_Field(derivNonEov, t(k)+dT, Xp+dT*K3, nSystem, nRows);

        [K1_OTD, k1_otd] = Velocity_Field_OTD(derivEov, t(k), Xp,            U,               Tp, nSystem, nRows, r);
        [K2_OTD, k2_otd] = Velocity_Field_OTD(derivEov, t(k)+dT/2, Xp+dT*K1/2,   U+dT*K1_OTD/2,   Tp+dT*k1_otd/2, nSystem, nRows, r);
        [K3_OTD, k3_otd] = Velocity_Field_OTD(derivEov,   t(k)+dT/2, Xp+dT*K2/2,   U+dT*K2_OTD/2,   Tp+dT*k2_otd/2, nSystem, nRows, r);
        [K4_OTD, k4_otd] = Velocity_Field_OTD(derivEov, t(k)+dT, Xp+dT*K3,     U+dT*K3_OTD,     Tp+dT*k3_otd ,nSystem, nRows, r);
...
function grad = gradientL(f, t, initPositions, nSystem, nRows)
    ...
    dFlowMap0 = reshape(eye(nSystem),nSystem*nSystem,1);
    %disp(size(initPositions));
    for i = 1:nRows
        pointForFullSystem = [initPositions(i,:), transpose(dFlowMap0)];

        rowvalue = f(t, pointForFullSystem);
        fullState = reshape(rowvalue(nSystem + 1 : end), [nSystem, nSystem]).';
        grad(:, :, i) = fullState;
    end
end
...
dFlowMap0 = reshape(eye(nSystem),nSystem*nSystem,1);
    ...
    for i=1:nRows
        pointForFullSystem = [X(i,:), transpose(dFlowMap0)];

        fullState =  f(t, pointForFullSystem);
        dy(i, :) = fullState(1:nSystem);
    end
end
