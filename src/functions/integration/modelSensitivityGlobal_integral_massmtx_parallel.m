function MS = modelSensitivityGlobal_integral_massmtx_parallel(derivative, MassMtx, initialPosition, timeSpan, toleranceFD, toleranceInt, Deltas)
...
t0 = timeSpan(1);
t = timeSpan(2);
funsq = @(s) vecFtlePullback(derivative, MassMtx, initialPosition, s, t, t0, toleranceFD);
...
function [sqrteig,cgtrace] = ftlePullback(dy,MassMtx,  x0, s, t, t0, toleranceFD)
    ...
    if(s ~= t0)
        x = ode45_vector_nonvectorized_massmtx_parallel(dy, MassMtx, [t0, s], x0, 'false'); % Advect the gridpoints to time s
    end
    if(s ~= t)
        [cgmax, cgtrace] = computeCGInvariantsode15s_parallel_massmtx(dy, MassMtx, x, [s, t], 'finiteDifference', 'false', toleranceFD);
    else
        cgmax = 1;
        cgtrace = numel(x0); 
    end
